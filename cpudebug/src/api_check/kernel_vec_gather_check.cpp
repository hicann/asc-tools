...
bool TikcppVecGatherCheck::CommonCheck()
{
    ...
    ASCENDC_CHECK(
        CheckTensorScope(param_.offsetLogicPos, static_cast<uint8_t>(HardWareIndex::UB), "offset", supportPos));
    ...
    ASCENDC_CHECK(CheckBufferSizeOverFlow(
        param_.offsetSize, GlobalParams::Instance().bufferSizeMap.at(param_.offsetPos),
        "check offset tensor buffersize failed"));
    // srcLocal tensor size is unknown. Only check srcBaseOffset will not exceed srcLocal size
    ASCENDC_CHECK_AND_LOG(param_.srcBaseOffset <= param_.srcSize, {
        CHECK_LOG_ERROR(
            "Failed to check srcBaseOffset value in "
            "%s, its valid range is 0 ~ %lu, current value is %u",
            apiName.c_str(), param_.srcSize, param_.srcBaseOffset);
    });
    return true;
}
...
bool TikcppVecGatherCheck::CheckAllHighLevel()
{
    ...
    ASCENDC_CHECK(
        CheckTensorOverflowHigh(param_.offsetDtypeBytes, param_.offsetSize, param_.calCount, "srcOffsetLocal"));
    return true;
}
