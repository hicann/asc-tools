...
bool TikcppVecBilinearInterpolationCheck::CheckTensorSize(std::vector<uint64_t> maskArray)
{
    ...
    ASCENDC_CHECK(CheckTensorSizeOverflow(
        expectedSize * param_.src1DtypeBytes, param_.src1Size, "src1Local", "BilinearInterpolation"));
...
bool TikcppVecBilinearInterpolationCheck::CheckAllLowLevel(std::vector<uint64_t> maskArray)
{
    ...
    ASCENDC_CHECK(
        CheckTensorScope(param_.src0LogicPos, static_cast<uint8_t>(HardWareIndex::UB), "src0Local", supportPos));
    ASCENDC_CHECK(CheckTensorScope(
        param_.offsetLogicPos, static_cast<uint8_t>(HardWareIndex::UB), "src0OffsetLocal", supportPos));
    ASCENDC_CHECK(
        CheckTensorScope(param_.src1LogicPos, static_cast<uint8_t>(HardWareIndex::UB), "src1Local", supportPos));
