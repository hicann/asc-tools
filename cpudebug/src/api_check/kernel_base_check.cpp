...
bool CheckTensorSizeOverflow(
    uint64_t expectedSize, uint64_t tensorSize, const std::string& tensorName, const std::string& apiName,
    const ModeType mode)
{
    std::string curMode = "";
    ...
    ASCENDC_CHECK_AND_LOG(expectedSize <= tensorSize, {
        CHECK_LOG_ERROR(
            "Failed to check %s size in %s%s, tensor size "
            "needs to be at least %lu bytes, while current tensor size is only %lu bytes.",
            tensorName.c_str(), apiName.c_str(), curMode.c_str(), expectedSize, tensorSize);
    });
    return true;
}
...
bool TikcppBaseCheck::UpdateMaskArrayAndCheck(std::vector<uint64_t>& maskArray, const uint32_t maxByteLen) const
{
    ...
    if (!MaskSetter::Instance().GetMask()) {
        ...
        CHECK_LOG_INFO(
            "Due to isSetMask = false, maskArray is changed to maskHigh %lu, maskLow %lu.", ModelFactoryGetMaskHigh(),
            ModelFactoryGetMaskLow());
    }
    ...
    if (maskArray.size() != 1 && maskArray.size() != 2) { // mask = len 1, mask[2] = len 2
        ASCENDC_CHECK_AND_LOG(
            false, { CHECK_LOG_ERROR("maskArray size is %lu, which should be 1 or 2.", maskArray.size()); });
        ...
        ASCENDC_CHECK_AND_LOG(CheckMaskImm(maskArray[0]), {
            CHECK_LOG_ERROR(
                "When maskArray size is 1, mask value %lu "
                "is invalid",
                maskArray[0]);
        });
    } else {
        ASCENDC_CHECK_AND_LOG(CheckMaskArray(maskArray), {
            CHECK_LOG_ERROR(
                "When maskArray size is 2, maskHigh %lu, "
                "maskLow %lu is invalid",
...
bool TikcppBaseCheck::CheckTensorScope(
    const uint8_t logicPos, const uint8_t expectedPos, const std::string& tensorInfo, const std::string& posInfo) const
{
    ...
    if (hwNameMap.find(hardPos) == hwNameMap.end() || hwNameMap.find(expectedPos) == hwNameMap.end()) {
        CHECK_LOG_ERROR(
            "the tensorPos/expectPos hardware pos not found, tensorPos index is %hhu, expectedPos is %hhu.", hardPos,
            expectedPos);
        return false;
    }
...
bool TikcppBaseCheck::CheckBufferSizeOverFlow(
    const uint64_t localSize, const uint64_t bufferSize, const std::string& errMsg) const
{
    if (localSize > bufferSize) {
        CHECK_LOG_ERROR(
            "%s, Allocated buffer size overflow, allocate buffer size is %lu, the limit size is %lu.", errMsg.c_str(),
...
bool TikcppBaseCheck::CheckMaskArray(std::vector<uint64_t> maskArray) const
{
    ...
    if (ModelFactoryGetMaskMode() == 1) {
        if (maskArray[1] > CONST_UINT32_MAX) { // only use maskLow[0:32] as counter mask. Only give warning!
            CHECK_LOG_WARNING(
                "Failed to check mask array in counter mode, maskLow must be in range 0 ~ %lu, current "
                "value is %lu",
                CONST_UINT32_MAX, maskArray[1]);
        }
        return true;
    }
    // normal mode
#if defined(__NPU_ARCH__) && (__NPU_ARCH__ == 1001 || __NPU_ARCH__ == 2002)
    ASCENDC_CHECK_AND_LOG(maskArray[0] != 0ULL || maskArray[1] != 0ULL, {
        CHECK_LOG_ERROR(
            "During calculation in "
...
bool TikcppBaseCheck::CheckMaskImm(const uint64_t mask) const
{
    ...
    if (ModelFactoryGetMaskMode() == 1) {
        if (mask > CONST_UINT32_MAX) { // only use mask[0:32] as counter mask. Only give warning!
            CHECK_LOG_WARNING(
                "Failed to check mask in counter mode, mask must be in range 0 ~ %lu, current value "
                "is %lu",
                CONST_UINT32_MAX, mask);
        }
        return true;
    }
    // normal mode
#if defined(__NPU_ARCH__) && (__NPU_ARCH__ == 1001 || __NPU_ARCH__ == 2002)
    ASCENDC_CHECK_AND_LOG(mask != 0ULL, {
        CHECK_LOG_ERROR(
            "During calculation in normal mode in Ascend910 / Ascend310p "
            "/ Ascend610, mask %lu cannot be 0.",
            mask);
    });
#endif
    return true;
...
void CounterSplitMainTail(
    std::vector<uint64_t>& maskArray, const uint32_t dtypeBytes, uint64_t& mainRepeatTimes, uint64_t& tailRepeatTimes,
    std::vector<uint64_t>& mainMaskArray, std::vector<uint64_t>& tailMaskArray)
{
...
uint64_t CalculateVectorMaxOffset(
...
uint64_t CalculateNeededTensorSize(
...
bool CheckTensorOverflowLowCounter(
    ...
    const std::string& apiName)
{
    ...
    uint64_t mainBlkSize =
        CalculateNeededTensorSize(mainMaskArray, params.dtypeSize, mainRepeatTimes, params.blkStride, params.repStride);
    ...
    if (tailRepeatTimes > 0) { // calculate tail block from the last repStride in main block
        ...
        uint64_t tailBlkSize = CalculateNeededTensorSize(
            tailMaskArray, params.dtypeSize, tailRepeatTimes, params.blkStride, params.repStride);
...
bool CheckTensorOverflowLowNorm(
    std::vector<uint64_t>& maskArray, const TensorOverflowParams& params, const std::string& tensorName,
    const std::string& apiName)
{
    uint64_t maxOffset =
        CalculateNeededTensorSize(maskArray, params.dtypeSize, params.repeatTimes, params.blkStride, params.repStride);
...
bool CheckTensorOverflowLowCounterGatherMask(
    std::vector<uint64_t>& maskArray, const TensorOverflowParams& params, const std::string& tensorName)
{
    ...
    ASCENDC_CHECK(
        CheckTensorSizeOverflow(maxOffset, params.bufferSize, tensorName, "GatherMask", ModeType::COUNTER_MODE));
    return true;
}
#endif
} // namespace
...
bool TikcppBaseCheck::CheckTensorOverflowLow(
    std::vector<uint64_t>& maskArray, const TensorOverflowParams& params, const std::string& tensorName) const
{
...
// check whether the needed size overflow the allocated buffersize, the unit is bytes.
bool TikcppBaseCheck::CheckTensorOverflowLowGathermask(
    std::vector<uint64_t>& maskArray, const TensorOverflowParams& params, const std::string& tensorName) const
{
...
bool TikcppBaseCheck::CheckTensorOverflowLowBrcb(
...
bool TikcppBaseCheck::CheckTensorAddrAlign(
    const uint64_t tensorAddr, const uint8_t phyPos, const uint64_t alignBytes, const std::string& tensorInfo) const
{
    uint64_t hardwareBaseAddr = static_cast<uint64_t>(
        reinterpret_cast<uintptr_t>(ConstDefiner::Instance().GetHardwareBaseAddr(static_cast<Hardware>(phyPos))));
    uint64_t tensorAbsPos = tensorAddr - hardwareBaseAddr;
    ASCENDC_CHECK_AND_LOG(((tensorAbsPos % alignBytes) == 0), {
        CHECK_LOG_ERROR(
            "Failed to check %s tensor address "
            "alignment in %s, current tensor address is %lu, which should be %lu byte aligned.",
            tensorInfo.c_str(), apiName.c_str(), tensorAbsPos, alignBytes);
    });
    return true;
}
