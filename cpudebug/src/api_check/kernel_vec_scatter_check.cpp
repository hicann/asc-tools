...
bool TikcppVecScatterCheck::CheckTensorSize(std::vector<uint64_t> maskArray)
{
    ...
    uint64_t maxOffset =
        ((param_.repeatTimes - 1) * DEFAULT_REPEAT_STRIDE + (blkNumLastRep - 1) * DEFAULT_BLK_STRIDE) * blockLen +
        eleNumLastBlk;
...
bool TikcppVecScatterCheck::CommonCheck()
{
    ...
    ASCENDC_CHECK(
        CheckTensorScope(param_.dstOffsetLogicPos, static_cast<uint8_t>(HardWareIndex::UB), "dstOffset", supportPos));
    ...
    ASCENDC_CHECK(CheckBufferSizeOverFlow(
        param_.dstOffsetSize, GlobalParams::Instance().bufferSizeMap.at(param_.dstOffsetPos),
...
bool TikcppVecScatterCheck::CheckAllLowLevel(std::vector<uint64_t> maskArray)
{
    ...
    TensorOverflowParams params = {
        ...
        static_cast<uint64_t>(param_.srcRepStride),
        false};
...
{
    ASCENDC_CHECK(CommonCheck());
    ASCENDC_CHECK(CheckTensorOverflowHigh(param_.dstDtypeBytes, param_.dstSize, param_.count, "dstLocal"));
    ASCENDC_CHECK(CheckTensorOverflowHigh(param_.srcDtypeBytes, param_.srcSize, param_.count, "srcLocal"));
    ASCENDC_CHECK(
        CheckTensorOverflowHigh(param_.dstOffsetDtypeBytes, param_.dstOffsetSize, param_.count, "dstOffsetLocal"));
    return true;
}
} // namespace check
} // namespace AscendC
