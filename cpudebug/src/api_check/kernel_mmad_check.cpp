...
bool TikcppMmadCheck::CheckMmadParamsRange(const uint32_t num, const std::string& paramName) const
{
    ...
    ASCENDC_CHECK_AND_LOG((num <= mmadParamRange), {
        CHECK_LOG_ERROR(
            "Failed to check %s value in %s, its valid range "
            "is 0 ~ 4095, current value is %u.",
            paramName.c_str(), apiName.c_str(), num);
    });
    return true;
}
...
bool TikcppMmadCheck::CheckMmadOverflow(const std::string& errMsg)
{
    ...
    if (needElementL0c > totalL0CSize) {
        CHECK_LOG_ERROR(
            "%s: "
            "needElementL0c(%u) is bigger than totalL0CSize(%u)",
            errMsg.c_str(), needElementL0c, totalL0CSize);
        return false;
    }
    ...
    if ((needElementL0b + needElementL0a) > totalL1Size) {
        CHECK_LOG_ERROR(
            "%s: "
            "needElementL0b(%u) + needElementL0a(%u) is bigger than totalL1Size(%u)",
            errMsg.c_str(), needElementL0b, needElementL0a, totalL1Size);
        return false;
    }
    return true;
}
