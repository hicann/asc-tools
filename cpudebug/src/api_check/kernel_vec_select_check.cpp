...
bool TikcppSelectBinaryCheck::CommonCheck()
{
    ...
    ASCENDC_CHECK(
        CheckTensorScope(param_.selMaskLogicPos, static_cast<uint8_t>(HardWareIndex::UB), "selMask", supportPos));
    ...
    ASCENDC_CHECK(CheckBufferSizeOverFlow(
        param_.selMaskSize, GlobalParams::Instance().bufferSizeMap.at(param_.selMaskPos),
        "check selMask tensor buffersize failed"));
    ASCENDC_CHECK(CheckBufferSizeOverFlow(
