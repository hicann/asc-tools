...
INSTALL_SCRIPT = (
    REPO_ROOT / "scripts/package/asc-tools/scripts/asc-tools_custom_install.sh"
)
UNINSTALL_SCRIPT = (
    REPO_ROOT / "scripts/package/asc-tools/scripts/asc-tools_custom_uninstall.sh"
)
CREATE_SOFTLINK_SCRIPT = (
    REPO_ROOT / "scripts/package/asc-tools/scripts/asc-tools_custom_create_softlink.sh"
)
REMOVE_SOFTLINK_SCRIPT = (
...
class TestOpTypeCollectorPackaging(unittest.TestCase):
    def test_wheel_declares_console_entry_for_bin_symlink_source(self):
        ...
        self.assertIn(
            "optype_collector=optype_collector.optype_collector_main:main", setup_py
        )
    ...
    def test_optype_collector_install_is_controlled_by_pylocal_like_msobjdump(self):
        ...
        self.assertIn(
            'installPyWhlLocal "optype_collector-0.1.0-py3-none-any.whl"', content
        )
        self.assertNotIn(
            'installMsprofWhlPackage "${install_path}/tools/optype_collector', content
        )

    def test_create_softlink_script_links_optype_collector_to_latest_bin_like_msopgen_msopst(
        self,
    ):
        content = CREATE_SOFTLINK_SCRIPT.read_text(encoding="utf-8")

        self.assertIn(
            'createCanndevSoft "$install_path" "$version_dir" "$latest_dir"', content
        )
        ...
        self.assertIn(
            'createSoftLink "$_src_dir" "$_dst_dir" "optype_collector"', content
        )

    def test_remove_softlink_script_removes_optype_collector_from_latest_like_msopgen_msopst(
        self,
    ):
        ...
        self.assertIn(
            'removeSoftLink "$_dst_dir" "optype_collector-0.1.0.dist-info"', content
        )
        self.assertIn(
            'rm -f "$install_path/$latest_dir/tools/optype_collector"', content
        )

    def test_uninstall_script_removes_optype_collector_bin_symlink_like_msopgen_msopst(
        self,
    ):
        content = UNINSTALL_SCRIPT.read_text(encoding="utf-8")

        self.assertIn(
            "module_arr_=(op_ut_run op_ut_helper msopst msopst.ini optype_collector)",
            content,
        )
        self.assertIn(
        ...
        self.assertIn(
            'removeSoftLink "${install_path}/${PLT_ARCH}-linux/bin/" "msopgen"', content
        )
        self.assertIn(
            'removeSoftLink "${install_path}/${PLT_ARCH}-linux/bin/" "msopst"', content
        )
        self.assertIn(
        ...
        self.assertNotIn(
            'removeSoftLink "${install_path}/bin" "optype_collector"', content
        )
    ...
    def test_docs_describe_usage_without_exposing_install_or_internal_soc_details(self):
        ...
        self.assertNotIn(
            "${install_path}/python/site-packages/bin/optype_collector", user_doc
        )
