...
namespace {
check::VecReduceWhlApiParams BuildReduceWhlParams(
    uint64_t dstAddr, uint64_t srcAddr, uint32_t dstDtypeSize, uint32_t srcDtypeSize, uint8_t repeatTimes,
    uint16_t dstRepStride, uint16_t srcBlkStride, uint16_t srcRepStride, ReduceOrder order, uint64_t dstSize,
    uint64_t srcSize, uint8_t dstPos, uint8_t srcPos)
{
    return check::VecReduceWhlApiParams{dstAddr,      srcAddr,      dstDtypeSize, srcDtypeSize, repeatTimes,
                                        dstRepStride, srcBlkStride, srcRepStride, order,        dstSize,
                                        srcSize,      dstPos,       srcPos};
}
} // namespace

INSTANTIATE_TEST_CASE_P(
    TEST_VEC_REDUCE_OTHER_WHL_CHECK, TestVecReduceOtherWhlCheckSuite,
    ::testing::Values(
...
TEST_P(TestVecReduceOtherWhlCheckSuite, TestCaseWholeReduceOther)
{
    ...
    auto chkParams = BuildReduceWhlParams(
        dst.addr, src.addr, dstDtypeSize, srcDtypeSize, repeatTimes, dstRepStride, srcBlkStride, srcRepStride,
        param.order, static_cast<uint64_t>(param.dstDataSize), src.length, LogicPos(dst), LogicPos(src));
...
TEST_F(TestVecReduceOtherWhlCheckSuite, WholeReduceOnlyValueBoundary)
{
    ...
    auto passParams = BuildReduceWhlParams(
        dstPass.addr, src.addr, sizeof(half), sizeof(half), repeatTimes, dstRepStride, srcBlkStride, srcRepStride,
        ReduceOrder::ORDER_ONLY_VALUE, 16, src.length, LogicPos(dstPass), LogicPos(src));
    ...
    auto failParams = BuildReduceWhlParams(
        dstFail.addr, src.addr, sizeof(half), sizeof(half), repeatTimes, dstRepStride, srcBlkStride, srcRepStride,
        ReduceOrder::ORDER_ONLY_VALUE, 14, src.length, LogicPos(dstFail), LogicPos(src));
...
TEST_F(TestVecReduceOtherWhlCheckSuite, WholeReduceOnlyIndexBoundary)
{
    ...
    auto passParams = BuildReduceWhlParams(
        dstPass.addr, src.addr, sizeof(half), sizeof(half), repeatTimes, dstRepStride, srcBlkStride, srcRepStride,
        ReduceOrder::ORDER_ONLY_INDEX, 32, src.length, LogicPos(dstPass), LogicPos(src));
    EXPECT_TRUE(CheckFunReduceOtherWhlImpl(passParams, mask, "WholeReduceMax"));

    auto dstFail = MakeTensor(TPosition::VECCALC, ALIGN_ADDR(30));
    auto failParams = BuildReduceWhlParams(
        dstFail.addr, src.addr, sizeof(half), sizeof(half), repeatTimes, dstRepStride, srcBlkStride, srcRepStride,
        ReduceOrder::ORDER_ONLY_INDEX, 30, src.length, LogicPos(dstFail), LogicPos(src));
    EXPECT_FALSE(CheckFunReduceOtherWhlImpl(failParams, mask, "WholeReduceMax"));
}
...
TEST_F(TestVecReduceOtherWhlCheckSuite, WholeReduceDtypeMismatch)
{
    ...
    auto chkParams = BuildReduceWhlParams(
        dst.addr, src.addr, sizeof(float), sizeof(half), 8, 1, 1, 8, ReduceOrder::ORDER_VALUE_INDEX, 32, src.length,
...
TEST_F(TestVecReduceOtherWhlCheckSuite, WholeReduceAddrAlignCheck)
{
    ...
    auto chkParams = BuildReduceWhlParams(
        dst.addr + 2, src.addr, sizeof(float), sizeof(float), 8, 1, 1, 8, ReduceOrder::ORDER_ONLY_VALUE, 32, src.length,
        LogicPos(dst), LogicPos(src));
