...
grep -Fq 'const char* socName = getSocName();' "${trace_runtime}" || \
    Fail 'trace runtime does not query the runtime SoC name'
grep -Fq 'aclsan::ResolveSocVersion(socName)' "${trace_runtime}" || \
