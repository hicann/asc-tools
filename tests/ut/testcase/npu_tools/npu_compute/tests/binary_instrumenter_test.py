...
if tool == "ld.lld":
    ...
    assert not (probe.parent / "probe.cpp").exists()
...
def main():
    with tempfile.TemporaryDirectory(prefix="npu-probe-test-") as directory:
        ...
        for name in ("ld.lld", "bisheng-tune"):
            script = tools / name
        ...
        assert "bisheng" not in calls, calls
        ...
        assert all(not pathlib.Path(path).exists() for path in caches + workdirs)
