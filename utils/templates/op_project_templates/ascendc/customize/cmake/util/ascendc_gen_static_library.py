...
def args_parse():
    ...
    parser.add_argument(
        "-f",
        "--object-files",
        help="Output files from host object targets, generated by cmake.",
    )
    ...
    parser.add_argument(
        "-t",
        "--tmp-obj-dir",
        help="Temporary dir for combining object files to static library.",
    )

    parser.add_argument(
        "-o",
        "--output-file",
        help="Output static library of current customize operator project.",
    )
    ...
    parser.add_argument(
        ...
        "--remove-tmp-files",
        default="1",
        help="Whether to remove temporary files.",
    )
...
def collect_object_from_files(object_files, tmp_obj_dir):
    ...
    if os.path.isfile(tmp_obj_dir):
        raise RuntimeError(
            "-t/--tmp-obj-dir is an existing file, is must be a directory, please check and reset."
        )
...
def unpack_kernel_library(kernel_libs, objects, tmp_obj_dir):
    ...
    for lib in kernel_libs_parts:
        ...
        objects += [
            x.strip() for x in output.decode("utf-8").split("\n") if x.strip() != ""
        ]
...
def pack_static_library(output_file, objects, tmp_obj_dir):
    ...
    for index in range(0, len(objects), step_size):
        run_command(
            ["ar", "qc", output_abs_file] + objects[index : index + step_size],
            cwd=tmp_obj_dir,
        )
