...
void AddFrameStack(
    NpusanReportCommon* common, std::uint32_t index, ReportStackRole role, const ReportFrame& frame)
{
...
void AddRawStack(
    NpusanReportCommon* common, std::uint32_t index, ReportStackRole role, const char* rawText)
{
    common->stacks[index].role = role;
...
NpusanMemcheckReport MakeInvalidAccessReport()
{
    ...
    AddFrameStack(&report.common, 0, ReportStackRole::kFaultDevice,
                  MakeFrame(0x4012, 0x12, "VectorAddKernel", "/workspace/vector_add.cpp", 87));
    AddRawStack(&report.common, 1, ReportStackRole::kHostLaunch,
...
NpusanMemcheckReport MakeApiErrorReport()
{
    ...
    AddRawStack(&report.common, 0, ReportStackRole::kHostLaunch,
...
NpusanInitcheckReport MakeInitcheckReport()
{
    ...
    report.common.pattern =
        static_cast<std::uint32_t>(NpusanInitcheckPattern::kPartialUninitializedRead);
...
NpusanRacecheckReport MakeRacecheckReport()
{
    ...
    AddFrameStack(&report.common, 0, ReportStackRole::kRelatedAccessA,
                  MakeFrame(0x6040, 0x40, "WriteTile", "/workspace/vector_add.cpp", 103));
    AddFrameStack(&report.common, 1, ReportStackRole::kRelatedAccessB,
                  MakeFrame(0x6088, 0x88, "ReadTile", "/workspace/vector_add.cpp", 118));
    return report;
}

NpusanReportExecContext MakeSyncExec(
    std::uint32_t coreId, std::uint32_t blockId, std::uint32_t pipeId, const char* pipeName,
    std::uint64_t pc, const char* function, std::uint64_t offset, const char* file,
    std::uint32_t line)
{
...
NpusanSynccheckReport MakeSynccheckBase(
    ...
    NpusanSyncPrimitiveKind primitiveKind, const char* triggerOperation,
    const NpusanReportExecContext& triggerExec)
{
    ...
    AddFrameStack(&report.common, 0, ReportStackRole::kSyncTrigger,
                  MakeFrame(triggerExec.pc, triggerExec.offset, triggerExec.function.c_str(),
                            triggerExec.file.c_str(), triggerExec.line));
    return report;
}

NpusanSynccheckReport MakeBarrierDivergenceReport()
{
    const NpusanReportExecContext trigger = MakeSyncExec(
        2, 17, 0, "SIMT", 0x4010, "BlockReduce", 0x40, "/workspace/reduce.cpp", 91);
    NpusanSynccheckReport report = MakeSynccheckBase(
        4001, ReportSeverity::kError, NpusanSynccheckPattern::kIntraCoreDivergent,
        NpusanSyncPrimitiveKind::kBarrier, "SYNC_THREADS", trigger);
    report.detailKind = NpusanSyncDetailKind::kBarrier;
    report.detail = NpusanSyncBarrierError{
        "Divergent execution entities in block", "block", 0x0000ffff, 0xffffffff, 0};
    report.common.stacks[0].frames.push_back(
        MakeFrame(0x3f20, 0x120, "ReduceKernel", "/workspace/reduce.cpp", 137));
    return report;
}
...
{
    const NpusanReportExecContext trigger = MakeSyncExec(
        1, 4, 1, "PIPE_S", 0x5040, "SignalMte", 0x40, "/workspace/pipeline.cpp", 55);
    const NpusanReportExecContext related = MakeSyncExec(
        1, 4, 1, "PIPE_S", 0x5010, "SignalMte", 0x10, "/workspace/pipeline.cpp", 48);
    NpusanSynccheckReport report = MakeSynccheckBase(
        4002, ReportSeverity::kError, NpusanSynccheckPattern::kPairingMismatch,
        NpusanSyncPrimitiveKind::kSetWaitFlag, "SET_FLAG", trigger);
    ...
    report.common.stacks[0].frames.push_back(
        MakeFrame(0x4f10, 0x90, "PipelineStage", "/workspace/pipeline.cpp", 103));
    AddFrameStack(&report.common, 1, ReportStackRole::kSyncRelated,
                  MakeFrame(related.pc, related.offset, related.function.c_str(), related.file.c_str(),
                            related.line));
...
{
    const NpusanReportExecContext trigger = MakeSyncExec(
        3, 8, 3, "PIPE_MTE3", 0x6080, "ReleaseBuffer", 0x80, "/workspace/buffer.cpp", 80);
    NpusanSynccheckReport report = MakeSynccheckBase(
        4003, ReportSeverity::kError, NpusanSynccheckPattern::kPairingMismatch,
        NpusanSyncPrimitiveKind::kGetRlsBuf, "RLS_BUF", trigger);
    report.detailKind = NpusanSyncDetailKind::kPairing;
    report.hasRelatedPoint = true;
    ...
    report.common.stacks[0].frames.push_back(
        MakeFrame(0x5f20, 0x60, "BufferStage", "/workspace/buffer.cpp", 104));
    return report;
}
...
{
    const NpusanReportExecContext trigger = MakeSyncExec(
        3, 8, 3, "PIPE_MTE3", 0x6040, "AcquireBuffer", 0x20, "/workspace/buffer.cpp", 70);
    NpusanSynccheckReport report = MakeSynccheckBase(
        4004, ReportSeverity::kWarning, NpusanSynccheckPattern::kPairingMismatch,
        NpusanSyncPrimitiveKind::kGetRlsBuf, "GET_BUF", trigger);
    report.detailKind = NpusanSyncDetailKind::kPairing;
    report.hasRelatedPoint = true;
...
{
    const NpusanReportExecContext trigger = MakeSyncExec(
        5, 2, 5, "MMA", 0x7080, "MmaStage", 0x80, "/workspace/gemm.cpp", 122);
    NpusanSynccheckReport report = MakeSynccheckBase(
    ...
    report.detail = NpusanSyncSequenceError{
        "Instruction order differs between warps", 3, 0xffffffff};
    report.common.stacks[0].frames.push_back(
        MakeFrame(0x6f00, 0x200, "GemmKernel", "/workspace/gemm.cpp", 188));
    return report;
}
...
{
    const NpusanReportExecContext trigger = MakeSyncExec(
        6, 1, 0, "SIMT", 0x8040, "BarrierWait", 0x40, "/workspace/barrier.cpp", 64);
    NpusanSynccheckReport report = MakeSynccheckBase(
        4006, ReportSeverity::kError, NpusanSynccheckPattern::kDeadlock,
        NpusanSyncPrimitiveKind::kSyncObject, "BARRIER_WAIT", trigger);
    ...
    report.common.stacks[0].frames.push_back(
        MakeFrame(0x7f10, 0x70, "ConsumerStage", "/workspace/barrier.cpp", 112));
    return report;
}
...
int main()
{
    ...
    const std::vector<NpusanReportRecord> records{
        NpusanReportRecord::From(invalidAccess),
        NpusanReportRecord::From(apiError),
        NpusanReportRecord::From(initcheck),
        NpusanReportRecord::From(racecheck),
        NpusanReportRecord::From(barrierDivergence),
        NpusanReportRecord::From(duplicateSet),
        NpusanReportRecord::From(unmatchedRls),
        NpusanReportRecord::From(unconsumedGet),
        NpusanReportRecord::From(sequenceMismatch),
        NpusanReportRecord::From(deadlock),
