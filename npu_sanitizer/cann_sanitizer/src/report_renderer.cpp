    ...
    static const std::vector<TemplateEntry> templates = {
        {{ReportTool::kMemcheck, "invalid_access"},
         {"========= {{Severity}}: Invalid {{space}} {{access}} of size {{accessBytes}} bytes\n"
          ...
          "=========     and is {{distanceBytes}} bytes {{before|after}} the nearest allocation at 0x{{base}} of size {{bytes}} bytes\n"}},
        ...
        {{ReportTool::kMemcheck, "api_error"},
         {"========= {{Severity}}: Program hit {{apiErrorName}} (error {{apiErrorCode}}) due to \"{{apiErrorMessage}}\" on ACL runtime API call to {{apiName}}.\n"}},
        ...
        {{ReportTool::kInitcheck, "partial_uninitialized_read"},
         {"========= {{Severity}}: Partially uninitialized {{space}} memory read of size {{accessBytes}} bytes\n"
          "=========     {{location}}\n"
          "=========     by aicore ({{coreId}}) block ({{blockId}}) pipe ({{pipeName}})\n"
          "=========     First uninitialized byte at 0x{{firstUninitAddress}}, {{uninitBytes}} bytes are uninitialized\n"}},
        {{ReportTool::kInitcheck, "unused_memory"},
         {"========= {{Severity}}: Unused {{space}} memory in allocation 0x{{base}} of size {{bytes}} bytes\n"
          "=========     Not written {{unusedBytes}} bytes at offset 0x{{firstUninitOffset}} (0x{{firstUninitAddress}})\n"
        ...
        {{ReportTool::kSynccheck, "inter_core_divergent"},
         {"========= {{Severity}}: Barrier error detected. {{reason}}.\n"
          "=========     trigger point: {{triggerOperation}} by aicore ({{triggerCoreId}}) block ({{triggerBlock}}) at {{triggerLocation}}\n"
          "{{relatedPointLine}}"
          "=========     scope {{scope}}, active mask 0x{{activeMask}}, expected mask 0x{{expectedMask}}\n"
          "{{objectLine}}"}},
        {{ReportTool::kSynccheck, "invalid_argument"},
         {"========= {{Severity}}: Synchronization error detected. {{reason}}.\n"
          "=========     trigger point: {{triggerOperation}} by aicore ({{triggerCoreId}}) block ({{triggerBlock}}) pipe ({{triggerPipe}}) at {{triggerLocation}}\n"
          "{{objectLine}}"}},
...
ReportRenderStatus AppendTemplate(
    const ReportTemplate& tpl, const ReportFields& fields, std::string* out)
{
    if (out == nullptr) {
        return ReportRenderStatus::kInvalidArgument;
    }
    ...
    const std::string& fieldPrefix, const std::string& locationKey, bool includeAt, ReportFields* fields)
{
    ...
    if (function != fields->end() && !function->second.empty()) {
        ...
        os << "pc 0x" << FieldOr(*fields, pcKey, "0") << " in "
           << FieldOr(*fields, kernelNameKey, "<unknown>");
    }
...
{
    return std::string("0x") + Hex(value);
}
...
void PutPrefixedExecFields(
    const NpusanReportExecContext& exec, const std::string& prefix, ReportFields* fields)
{
...
void PutSyncPointFields(
    const NpusanReportCommon& common, const NpusanSyncPoint& point, const std::string& prefix,
    ReportFields* fields)
{
...
bool ValidateReportCommon(
    const NpusanReportCommon& common, ReportTool expectedTool, std::uint32_t envelopePattern)
{
    if (common.tool != expectedTool || common.pattern != envelopePattern ||
        common.stackCount > kNpusanReportStackMax) {
        return false;
    }
...
{
    return lhs.launchId == rhs.launchId && lhs.binaryId == rhs.binaryId &&
           lhs.functionId == rhs.functionId && lhs.instrExecId == rhs.instrExecId &&
           lhs.serialNo == rhs.serialNo && lhs.pc == rhs.pc && lhs.offset == rhs.offset &&
           lhs.deviceId == rhs.deviceId && lhs.coreId == rhs.coreId && lhs.blockId == rhs.blockId &&
           lhs.blockType == rhs.blockType && lhs.pipeId == rhs.pipeId && lhs.siteId == rhs.siteId &&
           lhs.line == rhs.line && lhs.column == rhs.column && lhs.function == rhs.function &&
           lhs.file == rhs.file && lhs.pipeName == rhs.pipeName && lhs.kernelName == rhs.kernelName;
}
...
{
    return !point.hasExecContext && point.stackRole == ReportStackRole::kNone &&
           !point.operation.empty() && ExecContextsEqual(point.exec, NpusanReportExecContext{});
}

bool IsActualPoint(const NpusanSyncPoint& point)
{
    return point.hasExecContext && !point.operation.empty();
}
...
{
    if ((report.common.flags & kNpusanReportCommonHasExecContext) == 0 ||
        !IsActualPoint(report.triggerPoint) ||
    ...
    switch (pattern) {
        ...
        case NpusanSynccheckPattern::kDeadlock:
            return report.detailKind == NpusanSyncDetailKind::kObject &&
                   IsValidPrimitiveKind(report.primitiveKind) &&
                   std::holds_alternative<NpusanSyncObjectError>(report.detail) &&
                   (!report.hasRelatedPoint || IsActualPoint(report.relatedPoint) ||
                    IsExpectedPoint(report.relatedPoint));
...
ReportRecord ToReportRecord(const NpusanMemcheckReport& report)
{
    ...
    return ReportRecord{{ReportTool::kMemcheck, MemcheckPatternName(report.common.pattern)}, report.common.severity,
                        fields, ActiveCallStacks(report.common)};
}
...
ReportRecord ToReportRecord(const NpusanInitcheckReport& report)
{
    ...
    return ReportRecord{{ReportTool::kInitcheck, InitcheckPatternName(report.common.pattern)}, report.common.severity,
                        fields, ActiveCallStacks(report.common)};
}
...
ReportRecord ToReportRecord(const NpusanRacecheckReport& report)
{
    ...
    if (const ReportFrame* frame =
            FirstStructuredFrame(report.common, ReportStackRole::kRelatedAccessA)) {
        PutFrameLocationFields(*frame, "first", &fields);
    }
    if (const ReportFrame* frame =
            FirstStructuredFrame(report.common, ReportStackRole::kRelatedAccessB)) {
    ...
    } else if (report.common.pattern ==
               static_cast<std::uint32_t>(NpusanRacecheckPattern::kInvalidRemoteAccess)) {
    ...
    return ReportRecord{{ReportTool::kRacecheck, RacecheckPatternName(report.common.pattern)}, report.common.severity,
                        fields, ActiveCallStacks(report.common)};
}
...
std::string ActualRelatedPointLine(const NpusanSyncPoint& point, const ReportFields& fields)
{
    ...
    os << "=========     related point: " << OrUnknown(point.operation) << " by aicore ("
       << FieldOr(fields, "relatedCoreId", "<unknown>") << ") block ("
       << FieldOr(fields, "relatedBlock", "0") << ") pipe ("
...
ReportRecord ToReportRecord(const NpusanSynccheckReport& report)
{
    ...
    if (report.hasRelatedPoint && report.relatedPoint.hasExecContext) {
        ...
        fields["relatedPointLine"] = "=========     related point: expected " +
                                     OrUnknown(report.relatedPoint.operation) +
                                     ", but no runtime point was observed\n";
    }
    ...
    switch (report.detailKind) {
        ...
        case NpusanSyncDetailKind::kPairing: {
            ...
            if (detail.reason == NpusanSyncMismatchReason::kDuplicateOpen) {
                ...
                fields["relatedPointLine"] = "=========     related point: previous " +
                                             report.relatedPoint.operation + " at " +
                                             fields["relatedLocation"] + " is still pending\n";
                fields["expectedOperationLine"] = "=========     expected " + std::string(close) +
                                                  " before another " + open + "\n";
    ...
    return ReportRecord{{ReportTool::kSynccheck, SynccheckPatternName(report.common.pattern)}, report.common.severity,
                        fields, ActiveCallStacks(report.common)};
}
...
ReportRecord ToReportRecord(const NpusanSoccheckReport& report)
{
    ...
    return ReportRecord{{ReportTool::kSoccheck, SoccheckPatternName(report.common.pattern)}, report.common.severity,
                        fields, ActiveCallStacks(report.common)};
}
...
ReportRenderStatus ToReportRecord(const NpusanReportRecord& record, ReportRecord* out)
{
    ...
    switch (record.tool) {
        case ReportTool::kMemcheck: {
            ...
            if (report == nullptr ||
                !ValidateReportCommon(report->common, ReportTool::kMemcheck, record.pattern)) {
                return ReportRenderStatus::kInvalidArgument;
            }
        ...
        case ReportTool::kInitcheck: {
            ...
            if (report == nullptr ||
                !ValidateReportCommon(report->common, ReportTool::kInitcheck, record.pattern)) {
                return ReportRenderStatus::kInvalidArgument;
            }
            *out = ToReportRecord(*report);
            return ReportRenderStatus::kSuccess;
        ...
        case ReportTool::kRacecheck: {
            ...
            if (report == nullptr ||
                !ValidateReportCommon(report->common, ReportTool::kRacecheck, record.pattern) ||
        ...
        case ReportTool::kSynccheck: {
            ...
            if (report == nullptr ||
                !ValidateReportCommon(report->common, ReportTool::kSynccheck, record.pattern) ||
        ...
        case ReportTool::kSoccheck: {
            ...
            if (report == nullptr ||
                !ValidateReportCommon(report->common, ReportTool::kSoccheck, record.pattern)) {
                return ReportRenderStatus::kInvalidArgument;
            }
            *out = ToReportRecord(*report);
            return ReportRenderStatus::kSuccess;
...
bool StartsWithReportPrefix(const std::string& line)
{
...
const char* FrameLabel(ReportStackRole role)
{
    return IsHostStackRole(role) ? "Host Frame" : "Device Frame";
}
