...
`npu_sanitizer/demo/examples`. The tests quickly verify example compilation,
...
```bash
bash npu_sanitizer/demo/examples/memcheck/datacopy_stride/run.sh
```
...
```bash
bash npu_sanitizer/demo/examples/basic_func/multi_kernel/run.sh
...
```bash
bash npu_sanitizer/demo/examples/memcheck/matmul_leakyrelu_basic_api/run.sh
```
