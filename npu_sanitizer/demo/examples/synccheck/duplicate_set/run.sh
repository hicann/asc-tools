...
# 每个 AIC 对同一 flag 连续执行两次 SET_FLAG。
# 8 个实例都形成重复打开和未消费事件。
set +e
"${npu_check}" --tool synccheck -- build/demo
set -e

# 关注 summary：sync_events=16、synchronizations=1、matched_pairs=0、duplicate_opens=8，
# unmatched_closes=0、unconsumed_opens=8、errors=16。
# 同时必须报告 duplicate SET_FLAG 和 redundant SET_FLAG，并确认 child/session 生命周期完整。
