...
# 生成 [512, 128] * [128, 128] 的 FP16 输入；golden 为 FP32 Matmul 转 FP16 后执行 LeakyReLU。
# LeakyReLU 保留非负值，负值乘以 0.001。
(cd build && python3 ../scripts/gen_data.py)

# 从数据目录运行不应报错的 memcheck 示例。
set +e
(cd build && ../../../../build/npu_compute/bin/npu_check --tool memcheck -- ./demo)
set -e
...
printf '[PASSED] memcheck/matmul_leakyrelu_basic_api\n'
