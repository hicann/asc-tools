...
# 同一 stream 连续执行两个 AIV kernel，各留下一个未消费的 SET_FLAG，再统一同步一次。
set +e
...
# unmatched_closes=0、unconsumed_opens=2、errors=2。
# 两个事件都必须报告 redundant SET_FLAG，并确认 child/session 生命周期完整。
