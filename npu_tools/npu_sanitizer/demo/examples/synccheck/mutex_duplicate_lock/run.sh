...
# 同一 mutex 连续执行两次 GET_BUF：第二次为重复打开，结束时还留下 1 个未消费事件。
set +e
"${npu_check}" --tool synccheck -- build/demo
set -e

# 关注 summary：sync_events=2、synchronizations=1、matched_pairs=0、duplicate_opens=1，
# unmatched_closes=0、unconsumed_opens=1、errors=2。
# 同时必须报告 duplicate GET_BUF 和 redundant GET_BUF，并确认 child/session 生命周期完整。
