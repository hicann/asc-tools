...
# 8 个 AIV 实例各执行 1 次没有 WAIT_FLAG 的 SET_FLAG，形成 8 个未消费事件。
set +e
"${npu_check}" --tool synccheck -- build/demo
set -e

# 关注 summary：sync_events=8、synchronizations=1、matched_pairs=0、duplicate_opens=0，
# unmatched_closes=0、unconsumed_opens=8、errors=8。
# 同时必须报告 redundant SET_FLAG，并确认 child/session 生命周期完整。
