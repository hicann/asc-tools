...
`HardwareInfo.jsonl` 缺失或不是普通文件，CLI 返回采集错误码 3。采集结束时，空的
采集数据目录会被删除且不输出路径；包含部分采集文件的目录会被保留并输出路径。

目标程序中不支持再次启动 `npu-compute` 采集。检测到嵌套采集时，内层和外层均不
发布 REP，外层返回采集错误码 3，并输出：

```text
npu-compute: nested npu-compute collection is not supported
```

帮助、Section 列表和 REP 导入命令不属于采集命令。
...
发布普通文件，CLI 会返回采集错误码 3。采集数据目录包含部分文件时会被保留并输出路径。
